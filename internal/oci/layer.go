...
func (e *LayerExtractor) Apply(ctx context.Context, tb io.Reader, root string) error {
	tr := tar.NewReader(tb)
	for {
		select {
		case <-ctx.Done():
...
func NewWhiteoutHandler(hh HeaderHandler) *WhiteoutHandler {
	return &WhiteoutHandler{wrapped: hh, extracted: make(map[string]bool)}
}
...
func Extract(h *tar.Header, tr io.Reader, path string) error {
	...
	case tar.TypeReg:
		if err := ExtractRegularFile(h, tr, path); err != nil {
...
func ExtractRegularFile(h *tar.Header, tr io.Reader, path string) error {
	mode := h.FileInfo().Mode()
