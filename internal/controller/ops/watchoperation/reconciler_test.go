...
func TestWatchedControllerName(t *testing.T) {
	type args struct {
		name string
	}
	type want struct {
	...
	cases := map[string]struct {
		...
		"Success": {
			reason: "Should create controller name with watched prefix",
			args: args{
				name: "test-watch",
			},
			want: want{
				name: "watched/test-watch",
			},
		},
	}
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			got := WatchedControllerName(tc.args.name)
			if diff := cmp.Diff(tc.want.name, got); diff != "" {
				t.Errorf("\n%s\nWatchedControllerName(...): -want, +got:\n%s", tc.reason, diff)
			}
		})
	}
}
